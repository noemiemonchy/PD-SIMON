# Electrodes-averaged exponent - task (gp x cong) -------------------------------------
...
exp <- data %>%
  ...
  geom_dotplot(binaxis='y', stackdir='center', dotsize=0.5, 
               position = position_dodge(width = 0.9))+
  theme_minimal()+
  ylab(c("Electrodes-averaged exponent"))  +
  xlab(c(""))+
...
# create matrix with 3 columns (p group, p period, p interaction) and 199 rows
expo_results = matrix(nrow=199, ncol=3, byrow=TRUE)
...
# Do a two way repeated measure anova (group x period) on all electrodes
...
for (ELEC in 1:199){
  ...
  expo_results[ELEC,1] = res.aov$p[1]
  expo_results[ELEC,2] = res.aov$p[2]
  expo_results[ELEC,3] = res.aov$p[3]
  
}

? %>%
# apply FDR correction on results
expo_results_fdr = matrix(nrow=199, ncol=3, byrow=TRUE)
...
write.csv(expo_results_fdr, "expo_scalp_results_task_fdr.csv", row.names=TRUE)




# ELECTRODES-averaged offset - task (gp x cong) ---------------------------------------
...
off <- data %>%
  ...
  geom_dotplot(binaxis='y', stackdir='center', dotsize=0.5, 
               position = position_dodge(width = 0.9))+
  theme_minimal()+
  ylab(c("Electrodes-averaged offset"))  +
  xlab(c(""))+
  theme(#panel.border = element_rect(colour = "black", fill=NA, size=2),
    axis.line = element_line(colour = "black",size=1),
    axis.ticks = element_line(size=1,color="black"),
...
# ELECTRODES-specific offset - task (gp x cong) ---------------------------------------


# create array to store results
# create matrix with 3 columns (p group, p period, p interaction) and 68 rows
offset_results = matrix(nrow=199, ncol=3, byrow=TRUE)
...
for (ELEC in 1:199){
  ...
  offset_results[ELEC,1] = res.aov$p[1]
  offset_results[ELEC,2] = res.aov$p[2]
  offset_results[ELEC,3] = res.aov$p[3]
  
}

# apply FDR correction on results
offset_results_fdr = matrix(nrow=199, ncol=3, byrow=TRUE)
...
write.csv(offset_results_fdr, "offset_scalp_results_task_fdr.csv", row.names=TRUE)





# ELECTRODES-averaged exponent - period -----------------------------------------------

# Plot

data= as_tibble(read.table('scalp_aperiodic_param.csv', sep = ",", 
                           header = TRUE))
...
data = as_tibble(read.table('scalp_aperiodic_param_all.csv', 
                            sep = ",", header = TRUE))
...
for (ELEC in 1:199){
  ...
  expo_results[ELEC,1] = res.aov$ANOVA$p[1]
  expo_results[ELEC,2] = res.aov$ANOVA$p[2]
  expo_results[ELEC,3] = res.aov$ANOVA$p[3]
...
write.csv(expo_results_fdr, "expo_scalp_period_results_fdr.csv", row.names=TRUE)
...
# Electrodes-specific posthoc multiple comparisons
...
off_post_results = matrix(nrow=199, ncol=3, byrow=TRUE)
colnames(off_post_results) = c('post-pre','post-rest','rest-pre')

off_post_results_sign = matrix(nrow=199, ncol=3, byrow=TRUE)
...
for (ELEC in 1:199){
  ...
  off_post_results[ELEC, 1] = pwc$p.adj[1]
  off_post_results[ELEC, 2] = pwc$p.adj[2]
  off_post_results[ELEC, 3] = pwc$p.adj[3]
  off_post_results_sign[ELEC, 1] = pwc$statistic[1]
  off_post_results_sign[ELEC, 2] = pwc$statistic[2]
  off_post_results_sign[ELEC, 3] = pwc$statistic[3]
}

# apply FDR correction on results
off_post_results_fdr = matrix(nrow=199, ncol=3, byrow=TRUE)
...
for (ELEC in 1:199){
  ...
  offset_results[ELEC,1] = res.aov$ANOVA$p[1]
  offset_results[ELEC,2] = res.aov$ANOVA$p[2]
  offset_results[ELEC,3] = res.aov$ANOVA$p[3]
...
write.csv(offset_results_fdr, "offset_results_scalp_period_fdr.csv", row.names=TRUE)
